...
# Scan all projects
snyk test --all-projects --severity-threshold=low --package-manager=pnpm
```

**CI/CD Integration:**
- All pushes to `main` trigger automated Snyk security scans
- Build fails if any vulnerabilities are detected (severity threshold: low)
- Security scan runs before deployment

**Required Secret:**
- `SNYK_TOKEN` must be configured in GitHub repository secrets
...
Known vulnerabilities are patched using `pnpm.overrides` in `package.json`:
- Overrides force safe versions of transitive dependencies
